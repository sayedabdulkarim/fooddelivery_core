...
//component
import ShowSkeleton from "../../skeleton/homeSkeleton";
...
const TopRestaurantSection = ({ isLoadingHomePage }) => {
  ...
  const {
    ...
    isAtStart,
    isAtEnd,
  } = useImageCarousel(homePageData?.data?.topRestaurantList || []);

  return (
    <div
      className="home_best_offers"
      style={{ marginTop: isLoadingHomePage ? "50px" : "" }}
    >
      {isLoadingHomePage ? (
        <ShowSkeleton
